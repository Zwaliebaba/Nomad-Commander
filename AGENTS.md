...
What that change does **not** do is settle the question, because this file is not where it is settled. `Design/GameDesign.md` §13 still says the 3D client "is the reward for a working 2D game, the largest cost in the project and the least validated value, and it waits"; §15 lists "no 3D" among what v0.1 does not contain; the appendix records "2D first, 3D as reward" as **settled**. The GDD outranks this file on design (see the top of this document), and R23 binds whatever §15 says waits. So lifting the rule here makes 3D an engineering *option* rather than a sanctioned one — the owner moves GDD §13 and §15 when it becomes the second. A 2D map camera (pan and zoom over a map larger than the screen) was never a 3D question and is simply available.
