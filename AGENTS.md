...
3. **`Design/ADR/`** — engineering decisions taken while building, one file per decision (§6). Fifteen of them so far; it is the register, and `Plan/Roadmap.md`'s table of expected decisions is only a set of predictions against it.
