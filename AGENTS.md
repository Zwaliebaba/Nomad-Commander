...
3. **`Design/ADR/`** — engineering decisions taken while building, one file per decision (§6). Twenty-one of them so far; it is the register, and `Plan/Roadmap.md`'s table of expected decisions is only a set of predictions against it.
...
**The game never draws at any size but 1920×1080**, whatever the window is. The 80×45 cell grid, the 24-pixel line every text face is baked to (ADR-016), every layout in `Design/UI/UI-Spec.md` and every integer position in the map are unchanged and unconditional — a pass that asks the window how big it is has misunderstood this rule. One place knows: the present step, and it knows only how to fit a rectangle into another rectangle.
...
**Blending and samplers are a pass's own business, and the shared defaults stay opaque** (owner decision, 2026-09-16). Until then, blending, multisampling or a sampler in a new pass was "an ADR, not a pipeline field". It is now neither: a pass sets what it needs. `PipelineDefaults` exists since NC-022 and still turns blending and anti-aliased lines off, because opaque is the common case and the cheap one — a default, not a prohibition. `MeshPipeline` is the first pass to override one: it turns depth on and culls back faces, and changes nothing else.

Two things are worth knowing before reaching for one, and neither is a rule:

- **A sampler on text costs the 1:1 guarantee.** The glyph atlas is read with `Load()`, which takes integer texel coordinates and has no filtering to switch on, and a glyph is drawn at exactly the size it was baked — one texel a pixel (ADR-016). That is *why* a glyph reaches the glass as the rasterizer left it. Sample it at a non-integer position and it is blur, which is the thing R12's fixed unscaled screen exists to prevent. Sample what benefits from sampling; text does not. **Blending is a different matter and text does want it**: since ADR-016 a texel holds coverage rather than a bit, and the glyph pass blends the text colour by it — which is what anti-aliased type is, and is the one pass in the client that overrides `PipelineDefaults::Blend()`.
- **A back buffer cannot be multisampled, and that was never this file's doing.** Direct3D 12 supports only the flip-model swap effects, and DXGI does not multisample a flip-model back buffer — `SampleDesc.Count` must be 1, and the D3D12 swap-chain documentation says so in as many words: "multisampling a back buffer is not supported in D3D12". So `SampleDesc(1)` on the swap chain is a fact rather than a preference, and it still is.

  Text survives a present scale better since ADR-016 than it did before: an anti-aliased face resamples like any other picture, where an 8×8 bit pattern is the thing bitmap type is worst at. The 1:1 path is still the one to want.

  **The scene target is where multisampling becomes possible again.** DXGI still will not multisample a *back buffer*, but ADR-009 means nothing draws into one: a scene target may be created with `SampleDesc.Count` above 1 and resolved before the present step scales it. So the collision this bullet used to describe — a perspective map with a silhouette on every sphere, no MSAA and no way to get one — is gone. **Whether to pay for it was then decided, with a measurement: [ADR-013](Design/ADR/ADR-013-anti-aliasing-the-3d-map.md) says not yet.** NC-027 drew the map and measured it aliased — every one of 655 silhouette edge pixels has the background immediately outside it, so there is no partial coverage anywhere — and deferred anti-aliasing against a named trigger rather than a date: the first task that moves the camera or a fleet, because aliasing on a still picture is a jagged edge and aliasing on a moving one is crawl. Text survives multisampling when it comes, because a glyph is an axis-aligned quad on integer boundaries and its shape comes from `discard`.

**R13 — The executable ships alone.** There is no assets folder, no data directory, nothing beside `NomadCommander.exe` at runtime. Art, colours, fonts and sound are embedded as `constexpr` arrays in headers — the desk's type is three faces of IBM Plex Mono rasterized offline into coverage by `Tools/BakeFont.py` and committed as `NeuronClient/FontData.h` and `FontCoverage.h` ([ADR-016](Design/ADR/ADR-016-the-desk-text-faces.md)), and the icons are 8×8 bit art beside them. Nothing is loaded and no `.ttf` ships. **A generated header is committed, not built**, which is the opposite of `CompiledShaders/` and deliberately so: the tool needs Pillow, CI does not, and the bytes are reviewed like any other source. **Shaders are compiled at build time**, never at runtime: `<Library>/Shaders/<Shader>VS.hlsl` goes through the `.vcxproj`'s `FXCompile` step into `<Library>/CompiledShaders/<Shader>VS.h` as `g_<Shader>VS` (§2). No `D3DCompile`, no `d3dcompiler_47.dll` beside the executable, no `.cso` on disk. Never add a runtime file dependency, a working-directory assumption or a "just for development" loose-file path; the loose path is the one that ships.

**R13 binds a process acting as the CLIENT, and has exactly two sanctioned exceptions.** A process **acting as the host** may write **one universe store** — the file a universe is reloaded from, which is what "the universe runs continuously whether the player is present or not" (GDD §1, §7) costs — and **one instrumentation log**, the timestamped event stream from which the measured outcomes in GDD §15 are counted rather than guessed. Nothing else, and never a client. The store's form is settled: **[ADR-014](Design/ADR/ADR-014-the-universe-store-form.md) makes it a seed, a scenario id and a journal of every input with the tick it applied at — no snapshot**, on a measured 9 ms load of a thousand inputs against the two-second threshold at which one becomes necessary. Loading is replaying, which is what R16 buys and what makes the replay the receipt (GDD §4, §8). The log's format is **[ADR-015](Design/ADR/ADR-015-the-instrumentation-log-format.md)**. The exceptions are named rather than general: the next thing that wants to write a file is a new decision, not an inference from these.
...
**It binds what the executable is built from, not what a development tool needs.** `Build/*.py` and `Tools/*.py` never ship and never link; `Tools/BakeFont.py` needs Pillow and nothing else in the tree does, which is why ADR-016 could take it without reopening this rule. **Third-party *content* compiled in is a different question and it is the owner's**: art and fonts are allowed by R13, and anything under a licence needs the owner's approval before it lands, with the licence text travelling with the bytes (ADR-016 is the worked example).
