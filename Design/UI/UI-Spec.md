...
Cite as *UI §n*. Pixel values are given at 1920×1080 with the 1280×720 value in brackets; the two are the same layout on the same 80×45 cell grid (`GLYPH_SCALE` 3 vs 2).

## 1. Grid and chrome (every screen)

- Screen 1920×1080 [1280×720]. Cell 24 px [16 px]. Everything snaps to the cell; nothing draws at a non-integer position (NC-025).
...
- **Status line**, bottom, 48 px [32]: simulated day and time · rate control (`pause 1× 4× 24×`, active one bordered ACCENT) · credits · upkeep per day in WARNING · mothership state.
...
- **Projection**: dimetric, precomputed. Plane point (x, y) → screen (0.707·(x − y), 0.354·(x + y)) + centre. No camera, no zoom, no pan (AGENTS.md §5). The grid on the plane is 46 plane-px, drawn as two families of lines.
- **System**: a sphere (the isometric treatment) or a filled disc (flat fallback, same radius) in the owner's colour; radius 21 px [14], Kessel 27 px [18] as the selected/contested system; an oval ground shadow beneath; a thin owner-colour ring at radius 39 px [26] for harbours. Label to the right (or left for the two eastern systems): name in owner colour, `ROLE · owner` in TEXT_DIM beneath. Roles are text, never icons (NC-026).
