...
4. The ADR is decided: **1920x1080 on a 24-pixel cell**, recorded as [ADR-008](../ADR/ADR-008-the-screen-is-1920x1080-at-glyph-scale-3.md) (which spells it `GLYPH_SCALE` 3, the font of the day; ADR-016 kept the cell and removed the scale) and implemented. Its one open item is the measurement — nobody has yet confirmed `GetClientRect` on a real display.
...
- **On-screen copy is ASCII** (owner decision). Each face covers 0x20-0x7E with no fallback glyph, and the screens were authored with typographic characters. UI §6 carries the substitution table. **The PNGs still show the originals**: they are pictures and cannot be edited here, so where a PNG and `UI-Spec.md` disagree, the file wins (Agent-Prompt rule 1). Regenerating them from `Nomad Commander Desk.dc.html` would settle it.
- **The type is IBM Plex Mono, at the density these screens were drawn at** (ADR-016, NC-028). The package's own assumption line calls its type "loose fidelity: proportional mono type stands in for the 8x8 bitmap"; it no longer stands in for anything. Three faces, baked to the 24 px cell row, 12 px a Body character — which is what the mockups used, so a panel that reads on paper now reads on screen.
