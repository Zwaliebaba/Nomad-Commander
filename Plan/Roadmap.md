...
- **The wire schema lives in `GameLogic/Wire*.h`.** A `Wire*` header holds public aggregates (R8) with `Serialize`/`Deserialize` on `ByteWriter`/`ByteReader`, includes only NeuronCore, and never names `World`. Client-side files (all of NeuronClient, and all of NomadCommander except `App.cpp`) may include from GameLogic nothing but `Wire*.h`. The checker enforces the edge; the edge is what makes R18 structural.
