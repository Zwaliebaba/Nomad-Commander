...
| NC-049 | The map balances per good, and convoys go where the need is | GameLogic | S | NC-048 |
...
**NC-049 was added by NC-048 and is what made the third of those true of the generator rather than of one map.** The soak measured that every generated map ran a permanent daily deficit in at least one good, and NC-049 found that this was the smaller of two defects: the larger was that the convoy planner took the first surplus and the first deficit it found where its own header promised the deepest of each, so a warehouse stood at its cap destroying its production while a system two jumps away was dry for two simulated years. Both are fixed (**ADR-019**), and GDD §10's "stocks neither run away nor drain to zero" now holds across 48 maps over two simulated years, 8 over five, and Milestone 2's twenty-system shape — zero dry market-days in all of them.
...
**The identical-situation condition is met and measured: 81 percent, across all eight templates** (NC-060), holding at 80–85 across four objectives and three odds positions over five seeds. It is recorded here because it took a measurement to find that the first version of the affinity table *failed* it — comfortably passing on the one seed its test happened to use while giving 36 to 48 percent in four of twelve probed cases, because the rows summed unequally and one template won on arithmetic before any admiral's character was consulted. The remaining three conditions wait on NC-064, NC-065 and NC-067.
...
| Where the map's balancing term lives (**ADR-019**) | NC-049 | *Not foreseen by the plan, and **the recommendation was measured wrong and rejected**.* The plan proposed the unowned harbour. A convoy's source is always an empire's own system, so a harbour can neither export a surplus nor be a dependable destination for one: given the balancing term it stood at its cap destroying production or starved outright. It is a swap between goods at the **owned** systems instead, which keeps each system's totals balanced. The ADR also carries the larger defect the task found on the way — the convoy planner taking the first surplus and deficit rather than the deepest. | no |
| Detection and report noise (**ADR-020**) | NC-050 | *Taken, and sharpened by two things building it settled:* **detection follows movement rather than the clock** — a report is written when a fleet arrives, departs or drifts, which is what makes GDD §3's sighting nine hours old and what keeps a simulated year to 299 reports — and **a report is checked on counts, never on position**, because a fleet that moved is not a source that lied. | no |
| Reality and knowledge are two containers (**ADR-021**) | NC-051 | *Not foreseen by the plan, and written because **NC-050 broke a documented invariant and nothing noticed**.* `World.h` had said since NC-040 that belief is "held beside a World rather than inside one"; detection put its `Reports` table inside one, which compiled, passed 114 tests and both checkers. The ADR makes the separation structural: `World` is reality, `Knowledge` is everything anybody worked out, the seam between them is an `IncidentId`, and each half carries its own schema version and its own hash. | no |
| Battle resolution model (**ADR-022**) | NC-062 | *Taken whole, and sharpened by three things building it settled and one the owner decided.* Twelve rounds in three phases; a posture per template per phase; integer losses from strength against strength with the pinned spread; triggers delayed and fallible. The owner fixed the feel: **a lost battle is a bloody nose the loser withdraws from**, measured at 100 withdrawals and no annihilations in a hundred even fights. Building it needed a **cooldown** (two fleets sharing a system would otherwise be re-intercepted every tick and ground down in minutes, making that decision unreachable by arithmetic), a **retreat** (a withdrawing side takes a lane out, and one that cannot is caught again), and a **break threshold calibrated so that pursuit is what breaks a fleet** — at the first value tried nothing broke in a hundred hopeless fights and captures were unreachable code. | **yes** |
...
1. **GDD §3 and §6 do not reconcile arithmetically.** The Varn's fifty-eight percent in §3 is built from "detected within two jumps" (0.25), a hull-class match (0.15), the Oren denial (0.05 for others) and a route conflict (−0.30), which sum to 0.15. The number in §3 is illustrative. NC-052 tests the band (accuse but not act), not the figure; if the figure matters, §3 needs one more item of evidence or §6 a different weight.
   ...
   **NC-052 built the rule and found the follow-on question the finding could not have seen.** §6's three *positive sighting* rows — detected within two jumps (0.25), hull classes match (0.15) and testimony naming the suspect (0.30) — come to exactly 0.70, which is the act threshold. So the obvious way to make §3's number work, adding testimony to the case, does not raise it into the accusation band: it puts the empire straight past the window the section spends a paragraph describing. The task's test therefore builds the band from a sighting, a hull match and one prior (0.55) and says so. If the owner wants §3's fifty-eight to add up, the item that closes the gap has to be one of the small ones — the 0.05 others' denial, or a prior — rather than testimony, or §6's weights have to move.
