...
- **The empire and system names are already decided** (owner, 2026-09-16) and are what `Design/UI/screens/` draws: the third empire is `Sedu Compact`, and the systems beyond Kessel are `Harrow`, `Tessa Gate`, `Pale Anchor`, `Ashfall`, `Cinder Reach`, `Low Meridian` and `Sedu Hold`. Use them exactly; the header comment records where they came from rather than offering them up for renaming. Officer and admiral names beyond Varik are still this task's to invent and list.
